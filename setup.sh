...
if ! command -v starship &> /dev/null; then
  print_error "Starship is not installed. Please install starship before running this script"
fi
  ...
  absolute_source_path="$(pwd)/$source_path" # Get absolute path of source
        ...
        if [ -e "$destination_symlink" ] || [ -L "$destination_symlink" ]; then
          ...
          if [ -L "$destination_symlink" ] && [ "$(readlink "$destination_symlink")" = "$absolute_source_path" ]; then
            ...
            print_warning "Existing file or symlink found at '$destination_symlink'. Should it be backed up to '${destination_symlink}.bak'? (Y/N)"
            read confirm
